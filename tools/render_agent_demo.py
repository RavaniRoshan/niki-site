#!/usr/bin/env python3
"""Reusable renderer for Niki agent demo clips.

Renders TUI-style animated frames with Pillow + JetBrains Mono (matching the
existing public/demo.gif: 1200x750, dark charcoal #121111), then encodes
MP4 (H.264) + WebM (VP9) + optimized GIF + a static poster PNG per agent.

Usage:
    python3 tools/render_agent_demo.py            # all clips
    python3 tools/render_agent_demo.py planner    # one clip

Assets are written to public/agent-demos/<key>.{mp4,webm,gif,png}.
"""
...
def build_frames(spec):
    snaps = []  # list of (revealed_lines, artifact_or_None)
...
def draw_frame(revealed, artifact, cursor_on=True):
    ...
    d.text((W - 24, 11), "niki · session", font=FONT_BAR, fill=ASH, anchor="ra")
    # body
    y = Y0
    last = revealed[-1] if revealed else ("", "txt")
    for i, (text, color) in enumerate(revealed):
        is_last = i == len(revealed) - 1
        d.text((X0, y), text, font=FONT_REG, fill=C.get(color, TXT))
        if is_last and cursor_on:
            tw = FONT_REG.getlength(text)
            d.rectangle([X0 + tw + 2, y + 3, X0 + tw + 9, y + 18], fill=C.get(color, TXT))
        y += LH
    # artifact box
    if artifact:
        y += 8
        lines = artifact["lines"]
        box_h = 16 + 24 * (len(lines) + 1)
        bx, by, bw = X0, y, W - 2 * X0
        d.rounded_rectangle([bx, by, bx + bw, by + box_h], radius=6,
                            outline=BORDER, width=1)
        d.text((bx + 14, by + 12), artifact["title"], font=FONT_BOLD,
               fill=C.get(artifact["accent"], TXT))
        ly = by + 12 + 26
        for ln in lines:
            text = ln["t"] if isinstance(ln, dict) else ln[0]
            color = ln.get("c") if isinstance(ln, dict) else ln[1]
            d.text((bx + 14, ly), text, font=FONT_REG, fill=C.get(color, TXT))
            ly += 24
    return img
...
def render(key, spec):
    ...
    for i, (revealed, artifact) in enumerate(snaps):
        ...
        cursor_on = (i // 6) % 2 == 0
        img = draw_frame(revealed, artifact, cursor_on)
        f = os.path.join(FRAMES_DIR, f"{key}_{i:03d}.png")
        img.save(f)
        files.append(f)
        # poster = first frame that shows the artifact
        if artifact is not None and poster_src is None:
            poster_src = f
    if poster_src is None:
        poster_src = files[len(files) // 2]
    encode(key, files, poster_src)
    for f in files:
        os.remove(f)
    size = os.path.getsize(os.path.join(OUT_DIR, f"{key}.mp4"))
    print(f"  {key}: {len(snaps)} frames -> mp4/webm/gif/png ({size//1024} KB mp4)")
